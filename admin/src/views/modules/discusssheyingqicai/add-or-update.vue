	...
	<div class="addEdit-block" :style='{"padding":"30px","fontSize":"16px","color":"#666","background":"none"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-if="type!='info'"  label="评论内容" prop="content">
					<editor 
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else-if="ruleForm.content" label="评论内容" prop="content">
				    <span :style='{"border":"1px solid rgba(126, 96, 16, .2)","padding":"0 12px","color":"inherit","background":"#fff","display":"inline-block","fontSize":"16px","lineHeight":"40px","minWidth":"200px","fontWeight":"500"}' v-html="ruleForm.content"></span>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-if="type!='info'"  label="回复内容" prop="reply">
					<editor 
						style="min-width: 200px; max-width: 600px;"
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else-if="ruleForm.reply" label="回复内容" prop="reply">
				    <span :style='{"border":"1px solid rgba(126, 96, 16, .2)","padding":"0 12px","color":"inherit","background":"#fff","display":"inline-block","fontSize":"16px","lineHeight":"40px","minWidth":"200px","fontWeight":"500"}' v-html="ruleForm.reply"></span>
				</el-form-item>
...
export default {
    ...
    info(id) {
      this.$http({
        url: `discusssheyingqicai/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `discusssheyingqicai/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.discusssheyingqicaiCrossAddOrUpdateFlag = false;
											this.parent.search();
