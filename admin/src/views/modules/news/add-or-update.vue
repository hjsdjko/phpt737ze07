		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else-if="ruleForm.introduction" label="简介" prop="introduction">
					<span :style='{"border":"1px solid rgba(126, 96, 16, .2)","padding":"0 12px","color":"inherit","background":"#fff","display":"inline-block","fontSize":"16px","lineHeight":"40px","minWidth":"200px","fontWeight":"500"}'>{{ruleForm.introduction}}</span>
				</el-form-item>
...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.content = this.ruleForm.content.replace(reg,'../../../phpt737ze07/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
